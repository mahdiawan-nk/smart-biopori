...
export default function TrendChart({
    ...
    const avgTemp = validData.length
        ? validData.reduce((sum, item) => sum + (item.soil_temperature ?? 0), 0) / validData.length
        : 0;

    const avgMoisture = validData.length
        ? validData.reduce((sum, item) => sum + (item.soil_moisture ?? 0), 0) / validData.length
        : 0;
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <div className="mb-6 grid gap-4 md:grid-cols-2">
                <div className="rounded-2xl border border-slate-100 p-4 dark:border-zinc-800/60 bg-slate-50/50 dark:bg-zinc-900/40">
                    ...
                    <h2 className="mt-2 text-3xl font-bold text-slate-800 dark:text-zinc-100">
                        {avgTemp.toFixed(1)}°C
                    </h2>
                </div>
